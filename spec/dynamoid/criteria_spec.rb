...
describe "Dynamoid::Criteria" do
  before do
    ...
    @user3 = User.create(:name => 'Josh', :email => 'josh_0@joshsymonds.com')
  end
  ...
  it "finds last using where" do
    User.where(:name => 'Josh').last.should == @user3
    User.where(:name => 'Justin').last.should == @user2
  end

  it 'finds all using where' do
    User.where(:name => 'Josh').all.should == [@user1, @user3]
  end
  
  it 'returns all records' do
    User.all.should =~ [@user1, @user2, @user3]
  ...
  it 'passes each to all members' do
    User.each do |u|
      u.id.should == @user1.id || @user2.id || @user3.id
  ...
  it 'returns n records' do
    ...
    4.times { |i| User.create(:name => 'Josh', :email => 'josh_#{i}@joshsymonds.com') }
